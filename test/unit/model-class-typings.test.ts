    ...
    describe(name, () => {
      ...
      describe('Scope delegators', () => {
        it('understands all of the scope delegator methods', () => {
          ...
          const where : (opts : WhereClause) => Scope<typeof RootClass> = RootClass.where
          const order : (opts : SortScope) => Scope<typeof RootClass> = RootClass.order
          const selectExtra : (opts : FieldScope) => Scope<typeof RootClass> = RootClass.select
          const includes : (opts : IncludeScope) => Scope<typeof RootClass> = RootClass.includes
          const merge : (opts : Record<string, Scope>) => Scope<typeof RootClass> = RootClass.merge
          const stats : (opts : StatsScope) => Scope<typeof RootClass> = RootClass.stats
        })
        ...
        it('understands the scope finder methods', () => {
          ...
          const find : (id : string | number) => Promise<RecordProxy<JSORMBase>>
            = RootClass.find
          const all : () => Promise<CollectionProxy<JSORMBase>>
            = RootClass.all
